...
int main()
{
	...
	// expand the array of Phones by 1 element
	{
		...
		CellPhone* tempPhones = new CellPhone[count + 1];
		
		// TODO: copy elements from original array into this newly allocated array
		int i; // Control variable for the iteration
		for (i = 0; i < count; i++) { // Iterate through the original array.
			tempPhones[i] = pPhones[i]; // Allocates the value(s) to the new array.
		}
