...
namespace ama{
  ...
  // If the received name parameter is not valid, set it to a safe empty state.
  ...
  bool Product::operator>(const iProduct& right_operand) const{
    ...
    if( strcmp(p_name, right_operand.name()) > 0 ){
      // If true, change the control variable to true.
      is_greater = true;
    } // end if
  ...
  // Query function that returns the address of the string storing the Product's name.
  const char * Product::name() const{
    return p_name;
  } //end Product::name() const
