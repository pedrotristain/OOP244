...
namespace ama{
  ...
  istream& Date::read(istream& is){
    ...
    *this = Date(temp_year, temp_month, temp_day);
    ...
    if( is.fail() == true ){ // If reading fails, set the object to an safe empty state and the error status to 'error_input'
      //setSafe();
      status(error_input);
    } else { // If the reading is successfull, set the object member values accordingly and the error status to 'no_error'
      //set(temp_year, temp_month, temp_day);
      //clearError();
    } // end else
